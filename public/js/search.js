...
function renderPokeResult(result) {
  ...
  const cardMetaInfo = `data-set="${result.set}" data-setID=${result.id} data-image=${result.imageUrl} data-nationalPokeNum=${result.nationalPokedexNumber} data-rarity="${result.rarity}"`
  const htmlDiv = `
...
function handleImageClick() {
  ...
  $('.pokeImg').on('click', event => {
    ...
    $inputArray.forEach(item => {
      console.log(item);
      console.log(item.dataset.setid);
    })
